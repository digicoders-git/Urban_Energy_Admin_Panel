...
import { Users, Trash2, CheckCircle, XCircle, ChevronLeft, ChevronRight, Eye, X, Mail, Phone, MapPin, Hash, Wallet, Calendar, User } from 'lucide-react'
import { toast } from 'react-toastify'
import Swal from 'sweetalert2'
import { referrersApi } from '../api'

const S = {
  pending: { bg: 'rgba(255,184,0,0.12)', color: '#FFB800', border: 'rgba(255,184,0,0.25)', label: 'Pending' },
  active: { bg: 'rgba(34,197,94,0.12)', color: '#22c55e', border: 'rgba(34,197,94,0.25)', label: 'Active' },
  inactive: { bg: 'rgba(239,68,68,0.12)', color: '#ef4444', border: 'rgba(239,68,68,0.25)', label: 'Inactive' },
}
...
export default function Referrers() {
  ...
  const updateStatus = async (id, status) => {
    try {
      const updated = await referrersApi.updateStatus(id, status)
      setData(prev => prev.map(d => d._id === id ? updated : d))
      toast.success(`Referrer ${status === 'active' ? 'Approved ✓' : 'Rejected ✕'}`)
    } catch (e) { toast.error(e.message) }
  }

  const remove = async (id) => {
    const res = await Swal.fire({
      title: 'Delete Referrer?', text: 'This cannot be undone.', icon: 'warning', showCancelButton: true,
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Referrer <span className="glow-text">Management</span></h1>
          <p className="page-subtitle">{data.length} total registrations received</p>
        </div>
        <div style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 12, padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Users size={15} color="#10b981" />
          <span style={{ color: '#10b981', fontWeight: 700, fontSize: 13 }}>Referrer Portal</span>
        </div>
      </div>
      ...
      <div className="grid-4">
        {[
          ...
          { label: 'Active', val: counts.active, color: '#22c55e', bg: 'rgba(34,197,94,0.08)', border: 'rgba(34,197,94,0.15)' },
          { label: 'Inactive', val: counts.inactive, color: '#ef4444', bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.15)' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-stretch">
        ...
        <div className={`${selectedId ? 'lg:col-span-3' : 'lg:col-span-5'} transition-all duration-300 flex flex-col gap-4 relative z-10`}>
          <div className="glass shadow-xl border border-white/[0.08]" style={{ overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table className="tbl">
                <thead><tr><th>Name</th><th>Email</th><th>City</th><th>Referral Code</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                  {loading
                    ...
                    : paginated.length === 0
                      ...
                      : paginated.map((row) => {
                          const s = S[row.status] || S.pending
                          ...
                          return (
                            <tr key={row._id} onClick={() => setSelectedId(row._id)} style={{ cursor: 'pointer', background: isActive ? 'rgba(255,122,0,0.05)' : 'transparent' }}>
                              ...
                              <td style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13 }}>{row.email || 'N/A'}</td>
                              <td style={{ color: 'rgba(255,255,255,0.55)', fontSize: 13 }}>{row.city}</td>
                              <td><span className="badge" style={{ background: 'rgba(0,163,224,0.12)', color: '#00A3E0', border: '1px solid rgba(0,163,224,0.2)' }}>{row.referralCode}</span></td>
                              ...
                              <td>
                                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }} onClick={e => e.stopPropagation()}>
                                  {row.status !== 'active' && (
                                    <button onClick={() => updateStatus(row._id, 'active')} title="Approve"
                                  ...
                                  {row.status !== 'inactive' && (
                                    <button onClick={() => updateStatus(row._id, 'inactive')} title="Reject"
        ...
        {selectedId && selectedRow && (
          ...
          >
            <div style={{ position: 'absolute', top: -40, right: -40, width: 180, height: 180, background: 'radial-gradient(circle, rgba(16,185,129,0.18) 0%, transparent 70%)', pointerEvents: 'none', borderRadius: '50%', zIndex: 0 }} />
            ...
            <button 
              ...
              onMouseEnter={e => { e.currentTarget.style.color = '#10b981'; e.currentTarget.style.borderColor = 'rgba(16,185,129,0.3)'; e.currentTarget.style.background = 'rgba(16,185,129,0.08)' }}
            ...
            <div style={{ flex: 1, overflowY: 'auto', zIndex: 1 }} className="space-y-6">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, textAlign: 'left', paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', inset: -3, borderRadius: 16, background: 'linear-gradient(135deg, #10b981, #FFB800)', opacity: 0.35, filter: 'blur(5px)' }} />
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-xl text-white shadow-md flex-shrink-0"
                    style={{ background: 'linear-gradient(135deg,#10b981,#FFB800)', border: '1.5px solid rgba(255,255,255,0.25)', position: 'relative', zIndex: 1 }}>
                    {selectedRow.name?.[0] ?? '?'}
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.2, color: '#10b981', display: 'block', paddingLeft: 4 }}>Partner Overview</span>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 14 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.3)', uppercase: true }}>REFERRAL CODE</span>
                    <span style={{ fontSize: 13, fontWeight: 800, color: '#00A3E0', fontFamily: 'monospace' }}>{selectedRow.referralCode}</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.3)', uppercase: true }}>REGISTERED ON</span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.2, color: '#10b981', display: 'block', paddingLeft: 4 }}>Referrer Details</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  
                  {[
                    { label: "Phone Number", val: selectedRow.phone, icon: Phone, color: "#FF7A00" },
                    { label: "Email Address", val: selectedRow.email || 'N/A', icon: Mail, color: "#00A3E0" },
                    { label: "Linked City", val: selectedRow.city, icon: MapPin, color: "#22c55e" },
                    { label: "UPI Payout ID", val: selectedRow.upiId || 'Not Configured', icon: Wallet, color: "#eab308" }
                  ].map((item, idx) => (
                    <div key={idx} style={{ 
                      display: 'flex', 
                      alignItems: 'center', 
              ...
              {selectedRow.qrCode?.data && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                  <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.2, color: '#10b981', display: 'block', paddingLeft: 4 }}>Linked Payout QR</span>
                  <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, padding: 14, display: 'flex', gap: 16, alignItems: 'center' }}>
                    <div style={{ width: 80, height: 80, bg: 'white', background: 'white', borderRadius: 10, overflow: 'hidden', padding: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <img src={`${BASE}/referrers/qrcode/${selectedRow._id}`} alt="QR" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                    </div>
                    <div style={{ textAlign: 'left' }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#ffffff' }}>Active UPI QR Scanner</span>
                      <p style={{ margin: '4px 0 0 0', fontSize: 11, color: 'rgba(255,255,255,0.4)', lineHeight: 1.4 }}>Scan this QR code using any UPI application (PhonePe/GPay) to complete direct payout transfers.</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            {selectedRow.status === 'pending' && (
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 18, display: 'flex', gap: 12 }}>
                <button onClick={() => updateStatus(selectedRow._id, 'active')} style={{ flex: 1, padding: '12px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'rgba(34,197,94,0.15)', color: '#22c55e', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><CheckCircle size={15} /> Approve</button>
                <button onClick={() => updateStatus(selectedRow._id, 'inactive')} style={{ flex: 1, padding: '12px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.15)', color: '#ef4444', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><XCircle size={15} /> Reject</button>
              </div>
            )}
            {selectedRow.status !== 'pending' && (
